            ...
            <ul className={styles.itemList}>
              {items.map(item => {
                const discount = item.product.originalPrice
                  ...
                  : null;
                return (
                  <li key={`${item.product.id}-${item.selectedColor}`} className={styles.item}>
                    ...
                    <div className={styles.itemInfo}>
                      ...
                      <div className={styles.itemColor}>
                        ...
                        <span className={styles.colorHex}>{item.selectedColor}</span>
                      </div>
                      <div className={styles.itemBottom}>
                        <div className={styles.qtyRow}>
                          <button
                            className={styles.qtyBtn}
                            onClick={() => updateQty(item.product.id, item.selectedColor, -1)}
                          >−</button>
                        ...
                        <div className={styles.itemPriceCol}>
                          ...
                          {discount && (
                            <span className={styles.savedTag}>−{discount}%</span>
                          )}
                        </div>
                      </div>
                    </div>
        ...
        {items.length > 0 && (
          <div className={styles.footer}>
            <div className={styles.summaryRow}>
              ...
              <span className={styles.subtotalVal}>${totalPrice.toLocaleString()}</span>
            </div>
            <div className={styles.summaryRow}>
              <span>Shipping</span>
            ...
            <div className={`${styles.summaryRow} ${styles.totalRow}`}>
              <span>Total</span>
              <span className={styles.totalVal}>${totalPrice.toLocaleString()}</span>
            </div>
