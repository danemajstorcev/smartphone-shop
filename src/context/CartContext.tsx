...
function cartReducer(state: CartState, action: CartAction): CartState {
  switch (action.type) {
    case 'ADD': {
      ...
      return { items: [...state.items, { product: action.product, quantity: 1, selectedColor: action.color }] };
    }
    case 'REMOVE':
      return { items: state.items.filter(i => !(i.product.id === action.productId && i.selectedColor === action.color)) };
    case 'UPDATE_QTY': {
      return {
        items: state.items
          .map(i =>
            i.product.id === action.productId && i.selectedColor === action.color
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateQty = useCallback((productId: string, color: string, delta: number) => {
  ...
  return (
    <CartContext.Provider value={{ items: state.items, totalItems, totalPrice, addItem, removeItem, updateQty, clearCart, isInCart }}>
      {children}
