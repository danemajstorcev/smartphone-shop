...
const STEP_LABELS: Record<Step, string> = { info: 'Shipping', payment: 'Payment', review: 'Review', success: 'Done' };
    ...
    return (
      <div className={`page-wrapper ${styles.emptyPage}`}>
        <div className="container">
          ...
          <Link to="/shop" className="btn-primary">Shop Phones</Link>
        </div>
      </div>
    );
  }
      ...
      <div className={`page-wrapper ${styles.successPage}`}>
        <div className={styles.successBox}>
          ...
          <p className={styles.successSub}>
            Thank you, {form.firstName}! Your order will ship to <strong>{form.address}, {form.city}</strong>.
        ...
        <div className={styles.progress}>
          {STEPS.filter(s => s !== 'success').map((s, i) => (
            <div key={s} className={styles.progressStep}>
              <div className={`${styles.progressDot} ${stepIdx >= i ? styles.progressDotActive : ''} ${stepIdx > i ? styles.progressDotDone : ''}`}>
                {stepIdx > i ? (
              ...
              <span className={`${styles.progressLabel} ${stepIdx >= i ? styles.progressLabelActive : ''}`}>
                {STEP_LABELS[s]}
              </span>
              {i < 2 && <div className={`${styles.progressLine} ${stepIdx > i ? styles.progressLineDone : ''}`} />}
            </div>
          ))}
        </div>
        ...
        <div className={styles.layout}>
          ...
          <div className={styles.formArea}>
            ...
            {step === 'info' && (
              <div className={styles.card}>
                ...
                <div className={styles.formGrid}>
                  ...
                  <Field label="Email" error={errors.email} required className={styles.colFull}>
                  ...
                  <Field label="Street Address" error={errors.address} required className={styles.colFull}>
            ...
            {step === 'payment' && (
              <div className={styles.card}>
                ...
                {form.payMethod === 'card' && (
                  <div className={styles.formGrid}>
                    <Field label="Card Number" error={errors.cardNumber} required className={styles.colFull}>
                      <input
                    ...
                    <Field label="Cardholder Name" error={errors.cardName} required className={styles.colFull}>
                      <input value={form.cardName} onChange={e => set('cardName', e.target.value.toUpperCase())} placeholder="JOHN DOE" />
                    </Field>
                ...
                {form.payMethod === 'address' && (
                  <div className={styles.codNote}>
                    ...
                    <div>
                      ...
                      <p>Pay when your order arrives. No card required. Additional fee may apply.</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            {step === 'review' && (
              <div className={styles.card}>
                ...
                <div className={styles.reviewItems}>
                  {items.map(item => (
                    <div key={`${item.product.id}-${item.selectedColor}`} className={styles.reviewItem}>
                      <img src={item.product.image} alt={item.product.name} className={styles.reviewItemImg} />
                      <div className={styles.reviewItemInfo}>
                        <span className={styles.reviewItemName}>{item.product.name}</span>
                        <span className={styles.reviewItemColor}>
                          <span style={{ width: 10, height: 10, borderRadius: '50%', background: item.selectedColor, display: 'inline-block', border: '1px solid rgba(255,255,255,0.2)' }} />
                          Qty: {item.quantity}
                        </span>
                      </div>
                      <span className={styles.reviewItemPrice}>${(item.product.price * item.quantity).toLocaleString()}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className={styles.reviewMeta}>
                  <div className={styles.reviewMetaRow}>
                    <span>Ship to</span>
                    <span>{form.address}, {form.city} {form.zip}</span>
                  </div>
                  <div className={styles.reviewMetaRow}>
                    <span>Payment</span>
                    <span>{form.payMethod === 'card' ? `Card ending ${form.cardNumber.slice(-4)}` : 'Cash on Delivery'}</span>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className={styles.navBtns}>
              ...
              {step !== 'review' ? (
                ...
                >
                  ...
                  {placing ? 'Placing Order…' : `Place Order · $${totalPrice.toLocaleString()}`}
                </button>
              )}
            </div>
          </div>
          ...
          <aside className={styles.summary}>
            ...
            <div className={styles.summaryItems}>
              {items.map(item => (
                <div key={`${item.product.id}-${item.selectedColor}`} className={styles.summaryItem}>
                  ...
                  <span className={styles.summaryItemPrice}>${(item.product.price * item.quantity).toLocaleString()}</span>
                </div>
              ))}
            </div>
