...
const EXTRA_ROWS: ExtraRow[] = [
        ...
        <span className={styles.ratingStars}>
          {[1, 2, 3, 4, 5].map((i) => (
            <svg
              ...
              stroke={i <= Math.round(p.rating) ? "#ffb300" : "#5a5a8a"}
              strokeWidth="2"
            >
  ...
  },
  {
    ...
    render: (p) => (
      <span
        className={`${styles.typeChip} ${p.type === "flagship" ? styles.flagship : p.type === "mid-range" ? styles.midrange : styles.budget}`}
      >
        {p.type}
      </span>
    ),
...
function winnerForSpec(
  ...
  if (values.filter((v) => v === best).length === products.length)
    return new Set();
  return new Set(
    products
      .filter((p) => parseNumeric(p.specs[key]) === best)
      .map((p) => p.id),
  );
}

function winnerForExtra(
  row: ExtraRow,
  prods: Product[],
  lowerIsBetter = false,
): Set<string> {
  ...
  if (values.filter((v) => v === best).length === prods.length)
    return new Set();
  return new Set(
    prods.filter((p) => row.rawValue!(p) === best).map((p) => p.id),
  );
}
...
function PhoneHeader({
  ...
  return (
    <div
      className={`${styles.phoneHeader} ${isWinner ? styles.phoneHeaderWinner : ""}`}
    >
      {isWinner && (
      ...
      <div className={styles.phoneHeaderPrice}>
        ...
        {discount && (
          <span className={styles.headerDiscount}>−{discount}%</span>
        )}
      </div>
      <button
...
export default function Compare({ onCartAdd }: Props) {
  ...
  const slots = [
    ...compareList,
    ...Array(Math.max(0, 2 - compareList.length)).fill(null),
  ].slice(0, 4);
      ...
      <div className="container">
        ...
        <div className={styles.pageHeader}>
          <div>
            ...
            <p className={styles.pageSub}>
              Side-by-side spec comparison of up to 4 smartphones
            </p>
          </div>
        ...
        <div className={styles.tableWrap}>
          ...
          >
            <colgroup>
              ...
              {slots.map((_, i) => (
                <col key={i} className={styles.dataCol} />
              ))}
            </colgroup>
            ...
            <tbody>
              {ALL_CATEGORIES.map((category) => {
                ...
                const specInCat = SPEC_ROWS.filter(
                  (r) => r.category === category,
                );
                const allRows = [
                  ...extraInCat.map((r) => ({
                  ...
                  <React.Fragment key={category}>
                    ...
                    <tr className={styles.catHeaderRow}>
                      <td
                        ...
                        className={styles.catHeaderCell}
                      >
                        <span className={styles.catHeaderLabel}>
                          {category}
                        </span>
                      </td>
                    </tr>
                    ...
                    {allRows.map(({ type, row }) => {
                      ...
                      const values =
                        type === "spec"
                          ? filledProducts.map(
                              (p) => p.specs[(row as SpecRow).key],
                            )
                          : [];
                      if (showOnlyDiff && type === "spec" && allSame(values))
                        return null;
                          ...
                          {slots.map((product, i) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                {type === "spec"
                                  ? (product as Product).specs[
                                      (row as SpecRow).key
                                    ]
                                  : (row as ExtraRow).render(
                                      product as Product,
                                    )}
                              </td>
                            );
                ...
                <>
                  ...
                  <tr className={styles.verdictRow}>
                    ...
                    {slots.map((product, i) => {
                      if (!product)
                        return (
                          <td key={i} className={styles.emptyCell}>
                            —
                          </td>
                        );
                      ...
                      return (
                        <td
                          key={product.id}
                          className={`${styles.scoreCell} ${isTop ? styles.scoreCellTop : ""}`}
                        >
                          ...
                          <span
                            className={`${styles.scoreNum} ${isTop ? styles.scoreNumTop : ""}`}
                          >
                            {score}/100
                          </span>
                          {isTop && (
        ...
        {compareList.length === 0 && (
          <div className={styles.emptyState}>
            ...
            <p>
              Add phones using the search boxes above, or browse the shop and
              click Compare.
            </p>
