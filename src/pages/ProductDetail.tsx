        ...
        <div className={styles.layout}>
          ...
          <div className={styles.gallery}>
            <div className={styles.mainImg}>
              ...
              {product.isNew && <span className={`badge badge-new ${styles.galleryBadge}`}>New</span>}
              {discount && <span className={`badge badge-sale ${styles.galleryBadgeSale}`}>−{discount}%</span>}
            </div>
          ...
          <div className={styles.info}>
            <div className={styles.infoTop}>
              <Link to={`/shop?brand=${product.brand}`} className={styles.brand}>{product.brand}</Link>
              <span className={`badge ${product.type === 'flagship' ? 'badge-flagship' : ''}`}
                style={product.type !== 'flagship' ? { background: 'var(--surface)', color: 'var(--text-muted)', border: '1px solid var(--border)' } : {}}>
                {product.type}
              </span>
            </div>
            ...
            <div className={styles.priceRow}>
              ...
              {product.originalPrice && (
                <>
                  ...
                  <span className={`badge badge-sale`}>Save ${(product.originalPrice - product.price).toLocaleString()}</span>
                </>
              )}
            </div>
            ...
            <div className={styles.colorPicker}>
              <p className={styles.pickerLabel}>
                Color: <span style={{ color: selectedColor }}>{selectedColor}</span>
              </p>
            ...
            <div className={styles.actions}>
              ...
              <Link
                ...
                onClick={() => { if (!inCompare) addToCompare(product!); }}
            ...
            <div className={styles.deliveryBadges}>
              {['🚚 Free Shipping', '🔒 Secure Pay', '↩️ 30-Day Return'].map(b => (
                <span key={b} className={styles.deliveryBadge}>{b}</span>
              ))}
            </div>
          </div>
        </div>
            ...
            <div className={styles.reviewsPanel}>
              <div className={styles.reviewSummary}>
                <div className={styles.reviewScore}>
                  <span className={styles.bigScore}>{product.rating.toFixed(1)}</span>
                  <Stars rating={product.rating} />
                  <span className={styles.reviewTotal}>{product.reviewCount.toLocaleString()} reviews</span>
                </div>
              ...
              {[
                { name: 'Alex M.', score: 5, date: '3 days ago', text: 'Absolutely stunning device. The camera blows everything else out of the water. Battery easily lasts a full day. Worth every penny.' },
                { name: 'Sarah K.', score: 4, date: '1 week ago', text: 'Premium build quality and blazing fast performance. Runs everything I throw at it without breaking a sweat. Minor complaint: charging brick not included.' },
                { name: 'James T.', score: 5, date: '2 weeks ago', text: 'Best phone I\'ve ever owned. Display is incredible, speakers are loud, and the AI features are actually useful in everyday life.' },
              ].map((r, i) => (
                    ...
                    <div className={styles.reviewStars}>
                      {[1,2,3,4,5].map(n => (
                        <svg key={n} width="12" height="12" viewBox="0 0 24 24"
                          ...
                          stroke={n <= r.score ? '#ffb300' : '#5a5a8a'} strokeWidth="2">
                          <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>
                        </svg>
                      ))}
                    </div>
        ...
        {related.length > 0 && (
          <section className={styles.related}>
            <div className="section-header">
              <h2 className="section-title">More from <span>{product.brand}</span></h2>
              <Link to={`/shop?brand=${product.brand}`} className="section-link">All {product.brand} →</Link>
            </div>
            <div className="products-grid">
              {related.map((p, i) => (
                <ProductCard key={p.id} product={p} animDelay={i * 60} onAddToCart={onCartAdd} />
              ))}
            </div>
          </section>
        )}
