...
interface Props { onCartAdd: () => void; }
...
export default function Shop({ onCartAdd }: Props) {
  ...
  const activeCount =
    filters.brands.length + filters.types.length +
      ...
      <div className="container">
        ...
        <div className={styles.pageHeader}>
          <div>
            ...
            <p className={styles.pageSubtitle}>The latest flagship, mid-range and budget smartphones</p>
          </div>
        ...
        <div className={styles.layout}>
          {/* Filter panel */}
          <div className={`${styles.filterWrap} ${mobileFiltersOpen ? styles.filterOpen : ''}`}>
            <FilterPanel
              filters={filters}
              onChange={setFilters}
              onReset={handleReset}
          ...
          <div className={styles.productsArea}>
              ...
              <div className={styles.chips}>
                ...
                {filters.types.map(t => (
                  <span key={t} className={styles.chip}>
                    {t}
                    <button onClick={() => setFilters(f => ({ ...f, types: f.types.filter(x => x !== t) }))}>×</button>
                  </span>
                ))}
                <button className={styles.clearChips} onClick={handleReset}>Clear all</button>
              </div>
            )}
            ...
            {filtered.length === 0 ? (
              <div className="empty-state">
                ...
                <button className="btn-primary" onClick={handleReset}>Reset filters</button>
              </div>
            ) : (
